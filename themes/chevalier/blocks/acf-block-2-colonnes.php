...
function chevalier_2_colonnes_callback( $block ) {
	...
	if($largeur==="full") {
		$classLargeur="alignfull";
	} elseif($largeur==="medium") {
		$classLargeur="alignmedium";
	}
	printf('<section class="acf-block-deux-colonnes %s %s"><div class="image-container">%s</div>',
		$className,
		$classLargeur,
		wp_get_attachment_image( $image_id,'medium' )
	);
		echo '<div class="texte-container"><div class="texte-content">';
			if($titre) {
			...
			echo '</div>'; //fin decor
		echo '</div></div>'; //fin texte-content texte-container  
